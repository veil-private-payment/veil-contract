...
#[test]
fn test_hash_pair_rejects_field_element_outside_bn254() {
    let env = test_env();
    let admin = Address::generate(&env);
    let contract_id = env.register(ASPMembership, (admin, 3u32));
    let client = ASPMembershipClient::new(&env, &contract_id);
    let invalid = soroban_utils::bn256_modulus(&env);
    let right = U256::from_u32(&env, 2u32);

    assert!(client.try_hash_pair(&invalid, &right).is_err());
}
...
#[test]
fn test_insert_leaf_rejects_field_element_outside_bn254() {
    let env = test_env();
    let admin = Address::generate(&env);
    let contract_id = env.register(ASPMembership, (admin.clone(), 3u32));
    let client = ASPMembershipClient::new(&env, &contract_id);
    ...
    assert!(client.try_insert_leaf(&invalid).is_err());
