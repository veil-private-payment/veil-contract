...
//!
//! Reads a snarkjs `verification_key.json` and generates `vk.rs` in the Cargo
//! output directory. `lib.rs` includes that file at compile time, embedding the
//! verification key as static byte arrays with no persistent storage.
//!
//! By default the build embeds this repo's 2-input/2-output policy
//! transaction key from `circuits/keys/policy_tx_2_2_vk.json`. Set
//! `VERIFIER_VK_JSON` to override that key.
//!
//! # Usage
//!
//! ```bash
    ...
    let path = env::var("VERIFIER_VK_JSON")
        ...
        .unwrap_or_else(|_| {
            PathBuf::from(env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR not set"))
                .join("../../circuits/keys/policy_tx_2_2_vk.json")
        });
